 ...
 * @constructor
 */
const ShimmerSimpleGallery = ({ row, col, gap, className, fitOnFrame, imageType, imageHeight, caption, card }) => {
    const renderPhotos = () => {
        const photos = [];
        const items = row * col;
        ...
        for (let index = 0; index < items; index++) {
            photos.push(
                <div key={index}>
                    ...
                    {imageType === 'circular' && <ShimmerCircularImage size={imageHeight} center />}
                    {caption && (
                        <ShimmerTitle
                            variant="secondary"
                            className={classNames({
